...
console.log("=== 1. Redundant hero buttons removed ===");
{
  ...
  // No replacement button group under the destination copy.
  assert.doesNotMatch(
    hero,
    /rounded-full bg-emerald px-8 py-3\.5[\s\S]*?#quote/,
  );
...
{
  // Benefits list follows the destination paragraph with tighter spacing.
  assert.match(
    hero,
    /Republic of Ireland\.\s*<\/p>\s*<div className="mt-6 flex flex-wrap/,
  );
  assert.match(hero, /lg:mt-5 lg:grid lg:grid-cols-2/);
  assert.doesNotMatch(hero, /mt-10 flex flex-wrap gap-x-6/);
  assert.doesNotMatch(hero, /lg:mt-8 lg:grid lg:grid-cols-2/);
