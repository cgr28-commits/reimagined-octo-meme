...
// 4 BFS city (14 mi → S£44 E£50)
{
  ...
  assert.equal(s, expectedS);
  assert.equal(e, expectedE);
  add("City Hall → BFS", "airport", 14, s, e, "universal £44/£50");
}
...
// 8 Dublin city beyond airport
{
  ...
  assert.ok((s ?? 0) > 204 + 4, `Dublin city beyond must exceed DUB airport drop (£208), got £${s}`);
  assert.ok((e ?? 0) > (s ?? 0));
