...
console.log("\n=== 2. A2A underlying matches universal Saloon (DUB fixed added on airport path) ===");
{
  ...
  assert.equal(journey, 223, `168km universal Saloon must be £223, got £${journey}`);
...
console.log("\n=== 6. Ordinary address ↔ Dublin Airport (direction-aware fixed costs) ===");
{
  ...
  // Explicit City Hall 98 mi anchors: drop £208 / pick £213.
  assert.equal(calculateUniversalSaloonJourneyFareGbp(98) + DUB_DROP_FIXED, 208);
  assert.equal(calculateUniversalSaloonJourneyFareGbp(98) + DUB_PICK_FIXED, 213);
