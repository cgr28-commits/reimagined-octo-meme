...
/** Approved DUB interpolation (~98 mi → £204 Saloon). */
...
check("City Hall → Dublin: base £204 + M1 £4; drop-off fee £0; no removal", () => {
  const q = calculateQuote(CITY, "DUB", S, false, {}, DUB_M98, false)!;
  assert.equal(q.journeyFareGbp, 204);
  assert.equal(q.airportFixedCostsGbp, 4);
  assert.equal(q.amount, 208);
...
check("City Hall → Dublin RETURN: outbound toll £4 + return parking £5 + toll £4; 5% return", () => {
  ...
  assert.equal(ret.journeyFareGbp, roundGbp(getReturnJourneyFare(204))); // 387.6
  assert.equal(ret.airportFixedCostsGbp, 4 + 9); // outbound toll + return parking+toll
  assert.equal(ret.amount, roundGbp(387.6 + 13));
  const fees = resolveJourneyAirportFees({
    isAirportToAirport: false,
    airportCode: "DUB",
    fromAirport: false,
