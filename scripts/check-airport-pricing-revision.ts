...
// DUB 98 mi drop £234; BFS 14 mi £48
{
  ...
  assert.equal(dubCity?.amount, 208, "DUB Belfast centre drop-off = £204 + £4 M1");
  assert.equal(bfsCity?.amount, 44, "BFS City Hall 14 mi → £44");
...

{
  ...
  assert.equal(bfsCity?.amount, 44);
  assert.equal(bfsEstate?.amount, calculateUniversalEstateJourneyFareGbp(44));
