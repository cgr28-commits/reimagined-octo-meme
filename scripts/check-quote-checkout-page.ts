...
console.log("=== Checkout page is compact and sans-serif ===");
{
  ...
  assert.match(card, /function handleBackToQuote/);
  assert.match(card, /function renderBackToQuoteButton/);
  assert.match(card, /← Back to quote/);
  assert.match(card, /data-back-to-quote=\{placement\}/);
  assert.match(card, /quoteStep >= 2 \? renderBackToQuoteButton\("top"\)/);
  assert.match(checkout, /renderBackToQuoteButton\("bottom"\)/);
  assert.match(card, /function handleBackToQuote\(\) \{\s*navigateQuoteStep\(1\);/);
  assert.doesNotMatch(card, /history\.back\(|window\.history/);
  assert.match(card, /function checkoutAccessLine/);
  assert.match(card, /expressQuoteSelectionConfirmation/);
  ...
  assert.doesNotMatch(card, /Change drop-off|Change pick-up/);
  assert.match(card, /function checkoutAccessLine/);
  assert.match(checkout, /showChangeDropOff && expressEditingLeg != null/);
  assert.doesNotMatch(checkout, /renderExpressChoiceInPriceCard\("compact"\)/);
