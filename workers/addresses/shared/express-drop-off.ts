...
export function expressDropOffRecommendedLabel(
  ...
  void service;
  return `Express terminal — ${formatExpressDropOffGbp(fee)} included`;
}
...
export function expressCheckoutChangeLabel(
  _service: ExpressAirportService | "combined" = "drop-off",
): string {
  void _service;
...
export function expressQuoteExpressTitle(
  ...
  const product =
    service === "pick-up" ? "Express Terminal Pickup" : "Express Terminal Drop-Off";
  return `${product} — +${fee}`;
}
...
export function expressQuoteSelectionConfirmation(input: {
  ...
  if (input.service === "pick-up") {
    if (input.expressSelected) {
      return total
        ...
        : `✓ Express Terminal Pickup selected — ${fee} added.`;
    }
    return total
  ...
  if (input.expressSelected) {
    return total
      ...
      : `✓ Express Terminal Drop-Off selected — ${fee} added.`;
  }
  return total
...
/**
 * Payment / booking may continue when Express is kept, ineligible, or the
 * customer has explicitly chosen the free airport area. Selecting Free is
 * confirmation — a separate acknowledgement checkbox is not required.
 */
...
/** Every Express leg must be Express-selected or an explicit Free-area choice. */
